      ...
      <div className="space-y-6">
        <section className="grid gap-6 xl:grid-cols-[0.92fr,1.08fr]">
            ...
            <form className="mt-6 space-y-4" onSubmit={handleSubmit}>
              ...
              <label className="block">
                ...
                <textarea
                  ...
                  onChange={handleChange}
                  rows="6"
                  className="w-full rounded-2xl border border-border bg-surface px-4 py-3 text-[14px] leading-6 text-text-primary outline-none transition focus:border-primary focus:ring-4 focus:ring-primary/20"
              ...
              <div className="flex flex-wrap gap-3 pt-2">
                <button
                  type="submit"
                  disabled={submitting}
                  className="rounded-full bg-primary px-5 py-3 text-[14px] font-bold text-white transition hover:bg-primary disabled:cursor-not-allowed disabled:opacity-60"
                >
                ...
                <button
                  type="button"
                  onClick={resetForm}
                  className="rounded-full border border-border px-5 py-3 text-[14px] font-bold text-text-secondary transition hover:border-border hover:bg-background"
                >
                  Clear
                </button>
              </div>
          ...
          <SectionCard>
            <SectionHeader
              ...
              action={<p className="text-[14px] font-semibold text-neutral">{state.loading ? 'Loading...' : `${formatNumber(state.data.length)} sent`}</p>}
            />

            <div className="mt-6 grid gap-4 sm:grid-cols-2">
              <div className="rounded-3xl border border-border bg-background p-5">
                <p className="text-[12px] font-black uppercase tracking-widest text-neutral">Delivery mode</p>
                <p className="mt-3 text-[14px] font-semibold text-text-primary">Broadcast to all teachers</p>
                <p className="mt-2 text-[14px] leading-6 text-text-secondary">
                  Notifications are copied to every teacher account in the current institute.
                </p>
              </div>
              <div className="rounded-3xl border border-border bg-background p-5">
                <p className="text-[12px] font-black uppercase tracking-widest text-neutral">Read tracking</p>
                <p className="mt-3 text-[14px] font-semibold text-text-primary">Unread and read states</p>
                <p className="mt-2 text-[14px] leading-6 text-text-secondary">
        ...
        <SectionCard>
          <SectionHeader
            ...
            action={<p className="text-[14px] font-semibold text-neutral">{state.loading ? 'Loading...' : `${formatNumber(state.data.length)} batches`}</p>}
          />
            ...
            {state.loading ? (
              ...
              <div className="space-y-4">
                {state.data.map((notification) => (
                  <div key={notification.id} className="rounded-3xl border border-border bg-surface p-5 ">
                    <div className="flex flex-wrap items-start justify-between gap-4">
                      <div>
                        <div className="flex flex-wrap items-center gap-2">
                          <h3 className="text-base font-black text-text-primary">{notification.title}</h3>
                          <StatusBadge status="active" />
                        </div>
                        <p className="mt-2 text-[14px] leading-6 text-text-secondary">{notification.message}</p>
                      </div>

                      <div className="text-right">
                        <p className="text-[12px] font-black uppercase tracking-widest text-neutral">
                          {notification.timeLabel}
                        </p>
                        <p className="mt-2 text-[14px] font-semibold text-text-secondary">{notification.sentAtLabel}</p>
                      </div>
                    </div>

                    <div className="mt-5 grid gap-3 sm:grid-cols-3">
                      <div className="rounded-2xl bg-background px-4 py-3">
                        <p className="text-[11px] font-black uppercase tracking-widest text-neutral">Recipients</p>
                        <p className="mt-2 text-[18px] font-bold text-text-primary">{notification.recipientCount}</p>
                      </div>
                      <div className="rounded-2xl bg-background px-4 py-3">
                        <p className="text-[11px] font-black uppercase tracking-widest text-neutral">Read</p>
                        <p className="mt-2 text-[18px] font-bold text-emerald-600">{notification.readCount}</p>
                      </div>
                      <div className="rounded-2xl bg-background px-4 py-3">
                        <p className="text-[11px] font-black uppercase tracking-widest text-neutral">Unread</p>
                        <p className="mt-2 text-[18px] font-bold text-amber-600">{notification.unreadCount}</p>
                      </div>
                    </div>
                  </div>
                ))}
              ...
              <div className="rounded-2xl border border-dashed border-border bg-background px-6 py-10 text-center">
                <p className="text-[14px] font-bold text-text-primary">No notifications sent yet</p>
                <p className="mt-2 text-[14px] text-neutral">Create the first notification using the form above.</p>
              </div>
            )}
          </div>
        </SectionCard>
